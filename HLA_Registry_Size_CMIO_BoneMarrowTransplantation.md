...
**Cross-ethnic matching cannot substitute for same-ancestry recruitment.** With a combined pool weighted to Singapore's composition (77% Chinese), Malay, Indian and Others patients failed to reach 75% coverage below the 10⁷ ceiling under every regime; Chinese patients were servable but required roughly twice the same-ancestry figure. Donors are not interchangeable across ancestry groups, consistent with other multiethnic registries [3].
...
### 4.2 Why conventional uncertainty quantification did not detect it

Our bootstrap intervals were narrow — approximately ±200 donors around 42,847 for the Chinese group, or ±0.5%. They are correct as computed and misleading as a guide to total uncertainty, because they condition on the truncated distribution: resampling within a truncated support cannot recover deleted mass.

A further variance source sits outside them entirely: the EM input sample size. At a 10⁻⁴ floor the Chinese N* is 11,487,962 from 5,000 individuals and 3,153,571 from all 45,754, a 264% difference between nominally identical configurations, and the sample-size curve is non-monotonic, peaking at the cap. This is not sampling noise around a fixed target — it is the target moving as the rare tail becomes resolvable. Bootstrap intervals quantify sampling variability within a fixed pipeline and are silent on preprocessing and provenance, which here dominate; reporting them alone overstates precision, and headline registry sizes should be quoted to two significant figures at most.
