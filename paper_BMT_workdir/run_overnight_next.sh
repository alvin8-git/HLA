...
step "03 EM haplotypes (uncapped, 1e-6)" run03_1e6.log analysis python -u 03_hwe_test.py
say "   $(python - <<'PY'
import pandas as pd
...
step "04 registry model"        run04_uncapped.log analysis python -u 04_registry_model.py
step "13 cross-ethnic sensitivity" run13_uncapped.log analysis python -u 13_cross_ethnic_sensitivity.py
step "16 smoothing sensitivity" run16_uncapped.log analysis python -u 16_smoothing_sensitivity.py
step "10 LD report"              run10_1e6.log analysis python -u 10_ld_report.py
step "11 Others stratification" run11_1e6.log analysis python -u 11_others_stratification.py
step "07 GENE[RATE] validation" run07_uncapped.log analysis python -u 07_validate_em.py
step "12 match validation"      run12_uncapped.log analysis python -u 12_match_validation.py
step "08 section plots"         run08_uncapped.log analysis python -u 08_section_plots.py
step "14 pipeline flowchart"   run14.log       analysis python -u 14_pipeline_flowchart.py
step "15 EM convergence"       run15.log       analysis python -u 15_em_convergence.py
step "partial-match figures+CSV" pm_figs_1e6.log . python paper_BMT_workdir/partial_match_figs.py

# 3. Full bootstrap
say "-> 09 bootstrap B=1000 (expect ~3-4h)"
( cd analysis && HLA_BOOTSTRAP_B=1000 python -u 09_bootstrap_ci.py ) > $WD/run09_B1000.log 2>&1
RC=$?
if [ $RC -ne 0 ]; then
...
# 4. Manuscript
step "build v2.16" run_build_v216.log . python build_report.py
